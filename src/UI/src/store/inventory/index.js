...
export default {
    ...
    mutations: {
        ...
        setSelectedObjectX(state, obj) {
            state.selectedObjectX = obj;
        },
        ...
        setFastAccessWeapon(state, {key, from}){
            if(state.fastAccessWeapons[key] === undefined) return;
            if(from[0] == "inv"){
                if(from[1] != state.personalId) return;
                const item = state.inventories[state.personalId].items.find(i=>i.index == from[2]);
                if(!item || !item.isUsable()) return;
                state.fastAccessWeapons[key] = {type: "inv", id: item.id};
            }else if(from[0] == "eq"){
                if(from[1] != 1) return;
                state.fastAccessWeapons[key] = {type: "eq", id: from[2]};
            } else if(from[0] == "access"){
                const temp = state.fastAccessWeapons[from[1]];
                if(temp === undefined || state.fastAccessWeapons[key] === undefined) return;
                state.fastAccessWeapons[from[1]] = state.fastAccessWeapons[key];
                state.fastAccessWeapons[key] = temp;
                window.mp.trigger("cef:access:setWeaponButton", from[1], JSON.stringify(state.fastAccessWeapons[from[1]]));
            }
            window.mp.trigger("cef:access:setWeaponButton", key, JSON.stringify(state.fastAccessWeapons[key]));    
        },
        ...
        setFastAccessWeaponData(state, buttons){
            state.fastAccessWeapons = buttons;            
        },
    },
}
