export default {
  ...
  state: {
    messengerData: {
      ...
      Contacts: [
        // { Id: 12, Name: 'Амогус', Number: 88005543535, Time: 1130443200000, IsOnline: true },
      ],
      _cachedAccounts: [
        // { Id: 12, DisplayedName: 'факбой' }
      ],
      _cachedMessages: [
  ...
  actions: {
    ...
    /* SMS */
    sms_openChat: function ({ commit, state }, chatId) {
      const chat = state.messengerData.Chats.find(c => c.Id === chatId);
    ...
    sms_openPrivateChat: function ({ dispatch, commit, state }, accountId) {
      const privateChat = state.messengerData.Chats.find(c => c.AccountId === accountId);
      if (privateChat) {
        console.log('ny pizda')
        dispatch('sms_openChat', privateChat.Id);
      }
      else {
        // commit('msg_setCurrentChatIsLoading', true);
        commit('msg_setCurrentChat', {
