<template>
  ...
  BACK
      ...
      <div class="bank-modal_title"> Business payment</div>
      <div class="bank-modal_img">
        <img src="/img/bank/main/biz.png" alt="" />
        <img src="/img/bank/main/biz.png" alt="" />
      </div>
      <div class="bank-modal_descr">Business taxes are paid</div>
      <div class="bank-modal_value">
        <span
          ><span>{{ bizTaxBalance.toLocaleString("ru") }} /</span>&nbsp;<span>{{
            bizTaxMax.toLocaleString("ru")
          }}</span></span
        >
      </div>
      ...
      <button class="bank-modal_action bank-btn" @click="payForBiz">Transfer</button>
    </div>
  </div>
  ...
  <!-- <div class="bank-pay-for-biz bank-modal">
    <div class="bank-pay-for-biz__wrap bank-modal__wrap">
      ...
      <div class="bank-modal-desc">
        {{
          loc(
            `bank:menu:38@${(bizTaxMax - bizTaxBalance).toLocaleString("ru")}`
          )
        }}
      </div>
      <div class="bank-pay-for-biz__info bank-modal__block-shadow biz">
        <div class="bank-modal-prompt biz">{{ loc("bank:menu:39") }}</div>
        <div class="bank-pay-for-biz__balance">
          $ {{ bizTaxBalance.toLocaleString("ru") }}
          <span>/ {{ bizTaxMax.toLocaleString("ru") }}</span>
        </div>
      </div>
      ...
      <div class="bank-modal-btn btn-main" @click="payForBiz">
        {{ loc("bank:menu:41") }}
      </div>
    </div>
  </div> -->
