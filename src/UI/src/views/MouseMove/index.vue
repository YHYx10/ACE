  ...
  >
    ...
    >
      <div
        ...
        :key="index"
        v-show="move.showIcons[index]"
      >
        <img :src="`/img/mouseMove/${ico}.svg`" alt="ico" />
        <div>
          <span>{{ info[ico].title }}</span>
          <span>{{ info[ico].descr }}</span>
        </div>
      </div>
    </div>
...
export default {
    ...
    return {
      ...
      info: {
        cam: {
          ...
          descr: "Turn of the camera",
        },
        point: {
          title: "The right mouse button",
          descr: "The angle of the camera",
        },
        zoom: {
          title: "Mouse wheel ",
          descr: "Close/move away",
        },
        scroll: {
          title: "Mouse wheel",
          descr: "Close/move away",
  ...
  methods: {
    moveChar(e) {
      if (this.startLeftBtn) {
        ...
        if (direction.x !== 0 && this.move.values[0].enabled) {
          ...
          this.move.values[0].value = Math.min(
            Math.max(
              this.move.values[0].value +
                this.move.values[0].step * direction.x,
              this.move.values[0].min
            ),
            this.move.values[0].max
          );
          window.mp.trigger(
            this.move.values[0].callback,
            this.move.values[0].value.toFixed(3)
          );
        }
        if (direction.y !== 0 && this.move.values[1].enabled) {
          if (this.move.values[1].invert) direction.y *= -1;
          this.move.values[1].value = Math.min(
            Math.max(
              this.move.values[1].value +
                this.move.values[1].step * direction.y,
              this.move.values[1].min
            ),
            this.move.values[1].max
          );
          window.mp.trigger(
            this.move.values[1].callback,
            this.move.values[1].value.toFixed(3)
          );
        }
        ...
        if (direction.x !== 0 && this.move.values[2].enabled) {
          ...
          this.move.values[2].value = Math.min(
            Math.max(
              this.move.values[2].value +
                this.move.values[2].step * direction.x,
              this.move.values[2].min
            ),
            this.move.values[2].max
          );
          window.mp.trigger(
            this.move.values[2].callback,
            this.move.values[2].value.toFixed(3),
            this.move.values[0].value.toFixed(3)
          );
        }
        if (direction.y !== 0 && this.move.values[3].enabled) {
          if (this.move.values[3].invert) direction.y *= -1;
          this.move.values[3].value = Math.min(
            Math.max(
              this.move.values[3].value +
                this.move.values[3].step * direction.y,
              this.move.values[3].min
            ),
            this.move.values[3].max
          );
          window.mp.trigger(
            this.move.values[3].callback,
            this.move.values[3].value.toFixed(3)
          );
        }
      }
    },
    ...
    mousewheel(e) {
      ...
      this.move.values[4].value = Math.min(
        Math.max(
          this.move.values[4].value + this.move.values[4].step * direction,
          this.move.values[4].min
        ),
        this.move.values[4].max
      );
      window.mp.trigger(
        this.move.values[4].callback,
        this.move.values[4].value.toFixed(3),
        this.move.values[0].value.toFixed(3)
      );
    },
    click() {
      this.$store.commit("sounds/play", { name: "click", volume: 0.5 });
    },
  },
};
</script>
