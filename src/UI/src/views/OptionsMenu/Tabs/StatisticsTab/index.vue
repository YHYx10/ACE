<template>
  <div class="statistics-tab">
    ...
    <template v-if="!(isBusinessModal || isTransportModal || isHomeModal || showReferralModal)">
        ...
        <div class="username general-props">
          <div class="general-prop">
            ...
            <div class="general-prop_text">
              <div class="statistics-tab__left-title">Name Last name</div>
    ...
    <div class="statistics-tab__center">
      ...
      <button class="item family" @click="openFamily()">
        <div class="item__info">
          <div class="category">Category</div>
          <div class="title"> My family </div>
          <div class="subtitle" style="border: 2px solid rgba(92, 255, 128, 0.55); color: rgba(92, 255, 128, 1);" v-if="statistics.family">{{statistics.family}}</div>
          <div class="subtitle" v-else>Absent</div>
        </div>
      </button>
    </div>
    <div class="statistics-tab__right">
      <div class="item report">
        <div class="item__info">
          ...
          <div class="subtitle">You can contact the administration with all the questions you are interested in!</div>
        </div>
        <button class="report-btn" @click="setCurrentTab('ReportsTab')">Contact the administration</button>
      </div>
      <div class="extra">
        <extra-item
            ...
            :key="index"
            :item="item"
            @showInputModal="showInputModal = true"
            @showReferralModal="showReferralModal = true"
        />
      </div>
    </div>
    </template>
    ...
    <transition-group tag="div" name="modal">
      ...
      <home-modal :key="3" v-if="isHomeModal" @close="isHomeModal = false"/>
      <ModalInput :key="4" v-if="showInputModal" @close="showInputModal = false"/>
    </transition-group>
  </div>
</template>
...
export default {
  ...
  data: function () {
    return {
      ...
      infoItems: [
        {title: 'mmain:stats:info:number', icon: 'telephone', key: "phoneNumber"},
      ...
      extraItems: [
        ...
        {title: 'referral', icon: null, key: 'ref', text: 'referral'},
        {title: 'alert', icon: null, key: 'alert', keyFirst: 'code', keySecond: 'total', textFirst: 'Referral code', textSecond: 'Invited'},
        {title: 'Premium', icon: 'premium', icon1: 'premium1', key: 'premium', text: ''},
      ],
      actionItems: [
        {
          title: 'mmain:stats:house', bg: 'home', key: 'house', event: () => {
            this.isHomeModal = true;
            console.log('mmain:stats:house');
  ...
  methods: {
    ...
    openFamily: function () {
      window.mp.trigger("family:openFamilyInterface");
    },
