...
export default {
  ...
  methods: {
    ...mapMutations("auth", ['setIsRememberPass']),
    setRememberLogin(status) {
      this.setIsRememberPass(status)
      window.mp.trigger("auth:save:pass", this.auth.login, this.auth.password, this.isRememberPass);
    },
    // usePromo() {
