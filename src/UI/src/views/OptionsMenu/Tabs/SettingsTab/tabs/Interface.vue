<template>
  <div class="settings-interface">
    <div class="billet-item">
      <div class="title">{{ loc("mmain:setts:chat:censore") }}</div>
      <SettingsSwitch
          :status="settings.censore"
          class="settings-special-switch"
          @onSwitch="setSettingLocal($event, 'censore')
        "/>
    </div>
    <div class="billet-item">
    ...
    <div class="billet-item">
      <div class="title">
        ...
        <div class="settings-interface-input-body">
          ...
          <input type="number" disabled class="settings-interface-input" v-model="level" v-bind:style="{width: settings.muteLowLevelValue.toString().length + 'ch'}">
