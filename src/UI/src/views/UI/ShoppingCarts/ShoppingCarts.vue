  ...
  <div class="cart-list">
    ...
    <div class="list">
      <ShoppingCartItem
        ...
        :item="item"
        :index="index"
        @deleteItem="key => $emit('deleteItem', key)"
        @increase="key => $emit('increase', key)"
        @decrease="key => $emit('decrease', key)"
      />
    </div>
...
export default {
  ...
  computed: {
    shoppingCartTotal: function() {
      return this.list.reduce((total, shoppingCarProduct) => total + shoppingCarProduct.cost*shoppingCarProduct.count, 0)
    }
  },

  methods: {
    buyProducts: function() {
      if(this.list.length == 0 || this.shoppingCartTotal > this.money) return;
      this.$emit('onBuy', this.isPaymentBank)
    },
