<template>
  <div class="find-car-page">
    <div class="title">
      <div class="text">
        <span>Search in the database by</span>
    ...
    <template v-if="searchCar.name">
      <div class="search-result">
        <div class="record-dossier vehicle">
          <div class="record-avatar">
            <img src="/img/mdt/profile-placeholder.svg" alt="" />
          </div>
          <div class="record-summary">
            <span class="record-kicker">Vehicle profile</span>
            <strong>{{ searchCar.carModel }}</strong>
            <small>{{ searchCar.number }} / Owner: {{ searchCar.name }}</small>
          </div>
          <div class="record-status" :class="{ danger: searchCar.wantedLevel > 0 }">
            {{ searchCar.wantedLevel > 0 ? "FLAGGED" : "CLEAR" }}
          </div>
        </div>
        ...
        <div class="search-result__body">
          ...
          <div class="wanted">
            ...
            >
              <svg
                v-if="index + 1 <= searchCar.wantedLevel"
                width="20"
                height="19"
                viewBox="0 0 20 19"
                fill="none"
...
export default {
  ...
  data: function () {
    return {
      ...
      headNav: ["Pda_14", "Pda_15", "Pda_16", "Pda_17"],
      modalShow: false,
    };
  },
...
.find-car-page {
  ...
  .search-result {
    ...
    &__head {
      ...
      padding-left: conv(20);
      display: grid;
      grid-template-columns: conv(190) conv(86) conv(195) auto;
      column-gap: conv(20);
      font-weight: 700;
      font-size: conv(15);
      line-height: conv(18);
