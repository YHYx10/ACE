...
export function detectPrng(allowInsecure = false) {
    // Insecure as fuck, never use for something else, as in this project to generate call ids.
    return () => Math.random();
}
...
export function factory(currPrng) {
    ...
    return function ulid(seedTime = NaN) {
        if (isNaN(seedTime)) {
            seedTime = Date.now();
        }
