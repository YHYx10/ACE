...
//  }
 ...
 //  Functions
 function loadLuckyWheelAnimLib(){
     ...
     for(let i = 0; mp.game.streaming.hasAnimDictLoaded(animationLib) === 0 && i < 150; i++){
...
function loadLuckyWheel(){
     ...
     for (let index = 0; index < 150 && !mp.game.streaming.hasModelLoaded(luckyWheelHash); index++) {
        mp.game.wait(0);         
     }
     if(!mp.game.streaming.hasModelLoaded(luckyWheelHash)) return;
 ...
 function unloadLuckyWheel(){
    if(mp.objects.exists(luckyWheelObject)) 
